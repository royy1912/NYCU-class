...
class Dataset_Dance(torchData):
    def __init__(self, root, transform, mode='test', video_len=7, partial=1.0):
        ...
        ###############################################################################
        data_dirs = sorted(glob(os.path.join(root , f'test/test_img/*')))
        data_num = len(data_dirs)
        ##################################################################################
...
class Test_model(VAE_Model):
    ...
    def eval(self):
        ...
        df.to_csv(os.path.join(self.args.save_root, f'submission_new_tester_zero_z.csv'), header=True, index=False)
    ...
    def val_one_step(self, img, label, idx=0):
        ...
        x_hat = img[0][0].unsqueeze(0).to(self.args.device)  
        decoded_frame_list = []
        label_list = []
        ...
        for t in range(label.shape[0]):
            ...
            for _ in range(10):
                if self.args.test_zero_z:
                    z = torch.zeros(1, self.args.N_dim, 1, 1).to(self.args.device)
                else:
                    z = torch.randn(1, self.args.N_dim, 1, 1).to(self.args.device)
        ...
        self.make_gif(generated_frame[0], os.path.join(self.args.save_root, f'pred_seq{idx}_new_tester_zero_z.gif'))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--test_zero_z', action='store_true', help='Use z=0 during testing for deterministic output')
