...
class DQNAgent:
    ...
    def select_action(self, state):
        ...
        state_tensor = torch.from_numpy(np.array(state, dtype=np.uint8)).float().unsqueeze(0).to(self.device) / 255.0
        with torch.no_grad():
    ...
    def train(self):
        ...
        targets = compute_double_dqn_targets(
            self.q_net, self.target_net, next_states, rewards, dones, self.gamma
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--save-dir", type=str, default="./results_task3_DoubleduelingDQN")
    parser.add_argument("--wandb-run-name", type=str, default="Pong-run-task3_DoubleduelingDQN")
