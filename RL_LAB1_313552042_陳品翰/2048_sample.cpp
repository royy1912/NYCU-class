...
std::vector<int> average_episode;
std::vector<float>average_score;
/**
...
public:
	...
	virtual float estimate(const board& b) const {
		...
		for(int i = 0 ; i < 8 ; ++i)
		{
			...
			estimate_value = estimate_value + operator[](index);//把八個isomorphic的值都加起來
		}
	...
	virtual float update(const board& b, float u) {
		// TODO
		float u_split = u / 8;//平均分給8個同構
		float update_value = 0;
		for(int i = 0 ; i < 8 ; ++i)
		{
			size_t index = indexof(isomorphic[i],b);
			operator[](index) += u_split;
			update_value += operator[](index);
		}
		return update_value;
	}
...
protected:
	...
	size_t indexof(const std::vector<int>& patt, const board& b) const {
		...
		{
			index = index + (b.at(patt[i]) << (4 * i));//把board上pattern的位子的值記錄到index中，且因為每個數為4bit，所以一次左移4bit才不會影響到真正盤面的值。
		}
		return index;		
	}
...
public:
	...
	state select_best_move(const board& b) const {
		...
		for (state* move = after; move != after + 4; move++) {
			if (move->assign(b)) {
				...
				expect_value = expect_value + move->reward();
				board after_move_board;
				for(int i = 0 ; i < num ; i++)
				{
					after_move_board = move->after_state();
					//info << "after_move_board" << std::endl << after_move_board << std::endl;
					after_move_board.set(space[i],1);//如果i位子pop出來的是2
					//info << "after_move_board_1" << std::endl << after_move_board << std::endl;
					expect_value += 0.9 * estimate(after_move_board);
					after_move_board = move->after_state();
					after_move_board.set(space[i],2);//如果i位子pop出來的是4
					//info << "after_move_board_2" << std::endl << after_move_board << std::endl;
					expect_value += 0.1 * estimate(after_move_board);
	...
	void update_episode(std::vector<state>& path, float alpha = 0.1) const {
		...
		state front;
		state back;
		board back_before_board;
		board front_after_board;
		//path.pop_back();
		back = path.back();
		path.pop_back();
		front = path.back();
		...
		while(true)
		{
			back_before_board = back.before_state();
			front_after_board = front.after_state();
			
			
			//float target = front.reward() + estimate(back.before_state()); //r(next) + rate * V(s')
			float error = front.reward() + target - estimate(front.before_state());// r(next) + (rate * V(S')) - V(S);
			target = update(front.before_state(), error * alpha);
			back = front;
			path.pop_back();
			if(path.size() == 0)
				break;
	...
	void make_statistic(size_t n, const board& b, int score, int unit = 1000) {
		...
		if (n % unit == 0) { // show the training process
			...
			average_score.push_back(mean);
			average_episode.push_back(n);
...
int main(int argc, const char* argv[]) {
	...
	{
		for(auto &x :average_episode)
			output_x << x << "\n";
		output_x.close();
	}
	output_y.open("output_y.txt");
	if(!output_y.is_open())
	{
		info << "open file failed";
	}
