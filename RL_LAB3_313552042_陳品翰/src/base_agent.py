...
class PPOBaseAgent(ABC):
	...
	def train(self):
		...
		while self.total_time_step <= self.training_steps:
			...
			episode_idx += 1
			while True:
				action, value, logp_pi ,_= self.decide_agent_actions(observation)
	...
	def video_trigger(self , trigger):
		...
		if trigger == 1:
			...
			return False
		#return episode_idx % self.eval_interval == 0
	...
	def evaluate(self):
		...
		for i in range(self.eval_episode):
			...
			while True:
				...
				action, _, _ ,_= self.decide_agent_actions(observation, eval=True)
